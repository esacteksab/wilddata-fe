  ...
  if (module.load) {
    ...
    if (is_error) {
      /** @type {import('types/page').ErrorLoadInput} */ (load_input).status =
        status;
      /** @type {import('types/page').ErrorLoadInput} */ (load_input).error =
        error;
    }
...
async function respond(incoming, options, state = {}) {
  ...
  const method_override = incoming.url.searchParams
    .get(parameter)
    ?.toUpperCase();
  ...
  const print_error = (property, replacement) => {
    Object.defineProperty(request, property, {
      get: () => {
        throw new Error(
          `request.${property} has been replaced by request.url.${replacement}`
        );
      },
    });
  };
  ...
  try {
    return await options.hooks.handle({
      ...
      resolve: async (request, opts) => {
        ...
        const decoded = decodeURI(request.url.pathname).replace(
          options.paths.base,
          ""
        );
