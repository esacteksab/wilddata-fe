  ...
  constructor({ base, routes, trailing_slash, renderer }) {
    ...
    if (this.current_history_index === 0) {
      ...
      history.replaceState(
        { ...history.state, "sveltekit:index": 0 },
        "",
        location.href
      );
    }
...
class Renderer {
  ...
  async update(info, chain, no_cache, opts) {
    ...
    if (opts) {
      ...
      if (this.autoscroll) {
        const deep_linked =
          info.url.hash && document.getElementById(info.url.hash.slice(1));
        if (scroll) {
  ...
  async _load({ route, info: { url, path } }, no_cache) {
    ...
    const changed = this.current.url && {
      ...
      params: Object.keys(params).filter(
        (key) => this.current.params[key] !== params[key]
      ),
